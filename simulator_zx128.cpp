...
// --- ГЛОБАЛЬНАЯ ПАМЯТЬ ZX SPECTRUM 128K + TR-DOS ---
BYTE spec_rom[2][16384];  // ROM 0: 128К Меню/Editor, ROM 1: 48K BASIC
BYTE spec_ram[8][16384];  // 8 страниц ОЗУ по 16 Кб (Всего 128 Кб)
BYTE trdos_rom[16384];     // Самостоятельное ПЗУ системы TR-DOS

// Настройки диспетчеризации памяти (Порт 0x7FFD)
int current_ram_page = 0;  // Переключаемая страница ОЗУ в окне 0xC000-0xFFFF
int current_rom_page = 0;  // Текущая страница ПЗУ в окне 0x0000-0x3FFF
bool screen_shadow_page = false; // true = экран в RAM 7, false = RAM 5
bool memory_lock_128k = false;   // Блокировка режима 128К (Out 0x7FFD больше не работает)

// Состояние TR-DOS (Beta Disk Interface)
bool trdos_active = false; // Флаг: активно ли сейчас ПЗУ TR-DOS вместо обычного ROM
bool beta_ports_enabled = false; // порты ВГ93 доступны, пока не был полный сброс/выход из TR-DOS
...
// --- ГЛОБАЛЬНЫЕ ПЕРЕМЕННЫЕ ДЛЯ ЭМУЛЯЦИИ ДИСКОВОДА TR-DOS ---
std::vector<BYTE> current_trd_data; // Буфер сырого образа дискеты .TRD (обычно 655360 байт)
bool trd_is_inserted = false;       // Флаг: вставлен ли диск в дисковод A:
...
struct VG93 {
    ...
    BYTE sector_reg = 1;
    BYTE data_reg = 0;
    BYTE sector_buffer[256] = { 0 };
    int  transfer_pos = -1;
    int  transfer_len = 0;   // размер текущей передачи (256 для сектора, 6 для READ ADDRESS)
    bool last_cmd_type1 = true;
    bool multi_sector = false;    // бит m (0x10) команды READ/WRITE SECTOR — читать секторы подряд
    bool record_not_found = false; // RNF: сектор/адрес не найден (диск не готов / вне диапазона)
    BYTE system_reg = 0x00;
    BYTE current_id_sector = 1;
    bool step_direction_in = true;
    ...
    enum TransferType {
        TRANSFER_NONE,
        TRANSFER_READ_SECTOR,
        TRANSFER_WRITE_SECTOR,
        TRANSFER_READ_ADDRESS,
        TRANSFER_WRITE_TRACK   // новое
    };

    TransferType transfer_type = TRANSFER_NONE;

    void Reset() {
        // Полный сброс (включение питания / вставка диска) — сбрасывает
        // и саму микросхему ВГ93, и защёлку системного порта (сторона,
        // привод, HLT и т.д.).
        ResetChip();
        system_reg = 0x00;
    }

    // Сброс только внутренних регистров ВГ93 по сигналу CLR (запись 0
    // в бит 2 системного порта). На реальной плате CLR идёт на вход
    // RST самой микросхемы контроллера и НЕ трогает внешнюю защёлку
    // системного регистра (выбор стороны/привода) — она меняется
    // только явной записью в порт 0xFF. Если сбрасывать здесь и
    // system_reg, то запись, которая как раз и вызвала CLR (в ней же
    // заданы новые сторона/привод), немедленно затирается обратно в 0.
    void ResetChip() {
        track_reg = 0;
        sector_reg = 1;
        data_reg = 0;
    ...
    bool DiskReady() const {
        return trd_is_inserted && !current_trd_data.empty();
    }

    int Side() const
    {
        // Реальная разводка Beta 128: бит 4 порта 0xFF, инвертированный
        // (0 в бите => сторона 1, 1 в бите => сторона 0). Биты 6-7 при
        // ЗАПИСИ этого порта ничего не значат — это только читаемые
        // DRQ/INTRQ (см. ReadSystem), TR-DOS их никогда не выставляет.
        int side = 1 - ((system_reg >> 4) & 1);

        return side;
    }

    BYTE ReadStatus(WORD pc)
    {
        BYTE status = 0;
        ...
        if (!last_cmd_type1 && record_not_found)
            status |= 0x10;

        if (last_cmd_type1 && track_reg == 0)
            status |= 0x04;

        if (transfer_type != TRANSFER_NONE &&
            transfer_pos >= 0 &&
            transfer_pos < transfer_len)
        {
            status |= 0x03; // BUSY + DRQ
        }
        else if (last_cmd_type1 && DiskReady())
        {
            // Бит 1 в статусе Type I — INDEX (импульс "начало оборота диска").
            // Для Type II/III тот же бит означает DRQ, который уже
            // корректно выставляется веткой выше (0x03 во время активной
            // передачи) — здесь его подмешивать нельзя, иначе после
            // завершённой Type II/III команды (BUSY=0) время от времени
            // будет ложно взводиться "есть данные для чтения", хотя
            // передача давно закончена.
            //
            // Частота индекса привязана к эмулированным T-states
            // Z80 (audio_cpu_tstates), а не к реальному времени хоста
            // (GetTickCount64): дисковод 300 об/мин даёт период 200 мс =
            // 3 500 000 Гц * 0.2 с = 700000 T-states; импульс держим
            // активным первые ~5% периода (~10 мс), как и раньше.
            constexpr uint64_t INDEX_PERIOD_TSTATES = 700000; // 200 мс при 3.5 МГц
            constexpr uint64_t INDEX_PULSE_TSTATES = 35000;   // ~10 мс
            if ((audio_cpu_tstates % INDEX_PERIOD_TSTATES) < INDEX_PULSE_TSTATES)
                status |= 0x02;
        }
    ...
    BYTE ReadTrack(WORD pc) {
        return track_reg;
    }

    BYTE ReadSector(WORD pc) {
        return sector_reg;
    }

    size_t GetTRDOffset() const {
        const size_t logical_track =
            (size_t)track_reg * 2 + Side();

        const size_t sector =
            (size_t)(sector_reg - 1);

        return (logical_track * 16 + sector) * 256;
    }

    // Загружает сектор (sector_reg/track_reg) из образа диска в sector_buffer.
    // Возвращает false, если сектор недоступен (тогда чтение останавливается).
    bool LoadCurrentSector() {
        if (!DiskReady())
            return false;

        if (sector_reg < 1 || sector_reg > 16)
            return false;

        size_t offset = GetTRDOffset();

        if (offset + 256 > current_trd_data.size())
            return false;

        memcpy(sector_buffer, &current_trd_data[offset], 256);
        current_id_sector = sector_reg;

        return true;
    }

    BYTE ReadData(WORD pc) {
        if (transfer_type == TRANSFER_READ_SECTOR || transfer_type == TRANSFER_READ_ADDRESS) {
            if (transfer_pos >= 0 && transfer_pos < transfer_len) {
                data_reg = sector_buffer[transfer_pos++];
                drq = (transfer_pos < transfer_len); // Сбрасываем DRQ только если прочитан последний байт

                if (transfer_pos >= transfer_len) {
                    if (multi_sector && transfer_type == TRANSFER_READ_SECTOR) {
                        sector_reg++;

                        if (sector_reg > 16) {
                            // Как и в WriteData: реальная ВГ93 не переходит на другую
                            // дорожку сама, а завершает команду с RNF по окончании
                            // текущей дорожки. Переход на следующий трек — исключительно
                            // задача софта TR-DOS (явный SEEK), автопродолжение здесь
                            // рассинхронизирует внутренний учёт TR-DOS с фактическим
                            // положением "головки" эмулятора.
                            EndCommand(true);
                        }
    ...
    BYTE ReadSystem(WORD pc)
    {
        BYTE sys = system_reg & 0x3F;

        if (drq)
            sys |= 0x40; // Бит 6: DRQ (1 = данные готовы к чтению)

        if (intrq)
            sys |= 0x80; // Бит 7: INTRQ (1 = команда полностью завершена)
    ...
    void WriteCommand(BYTE val, WORD pc) {
        ...
        // ПЕРЕХВАТ КОМАНДЫ FORCE INTERRUPT (0xD0 - 0xDF) ---
        if ((val & 0xF0) == 0xD0) {
            last_cmd_type1 = true;
            transfer_pos = -1;
            transfer_len = 0;
            ...
            busy = false;          // Команда 0xD0 ОБЯЗАНА немедленно гасить busy!
            intrq = true;          // ВАЖНО: раньше bit7 в ReadSystem брался из !busy,

            return;                // Завершаем выполнение, не давая упасть коду вниз
        }
        ...
        if ((val & 0x80) == 0) {
            ...
            EndCommand(false); // Type I завершаются быстро
            return;
        }
        ...
        // -------------------------
        if ((val & 0xC0) == 0x80) {
            last_cmd_type1 = false;
            busy = true; // Контроллер занят чтением/записью

            multi_sector = (val & 0x10) != 0;
            transfer_pos = 0;
            transfer_len = 256;
            ...
            if ((val & 0x20) == 0) {
                ...
                if (LoadCurrentSector()) {
                    transfer_type = TRANSFER_READ_SECTOR;
                    drq = true; // Данные готовы в буфере
                }
                else {
                    EndCommand(true); // Сектор не найден
                }
            }
            else {
                // WRITE SECTOR
                if (DiskReady() && sector_reg >= 1 && sector_reg <= 16) {
                    memset(sector_buffer, 0, 256);
                    transfer_type = TRANSFER_WRITE_SECTOR;
                    drq = true; // Готовы принимать байты от процессора Z80
                }
                else {
                    EndCommand(true);
                }
        ...
        // -------------------------
        // TR-DOS выполняет эту команду после каждого SEEK, чтобы
        // проверить/скорректировать фактическую позицию головки:
        // читает 6 байт (track, side, sector, length-code, CRC1, CRC2)
        // и затем сама записывает полученный track обратно в регистр.
        // Без реализации этой команды ReadData возвращал "протухшее"
        // значение data_reg от прошлой операции — по случайности
        // совпадавшее с нужным треком на первых секторах, но
        // расходившееся дальше, из-за чего TR-DOS переставлял
        // track_reg на неверный трек и грузил не те сектора.
        if ((val & 0xF0) == 0xC0) {
            last_cmd_type1 = false;
            busy = true;
            ...
            // В TRD первый ID на каждой дорожке — сектор 1.
            const BYTE id_track = track_reg;
            const BYTE id_side = (BYTE)Side();
            const BYTE id_sector = current_id_sector;

            sector_buffer[0] = id_track;
            sector_buffer[1] = id_side;
            sector_buffer[2] = id_sector;
            sector_buffer[3] = 1;   // 256 bytes
            sector_buffer[4] = 0;   // CRC1
            sector_buffer[5] = 0;   // CRC2

            // Особенность WD1793/VG93:
            // после READ ADDRESS track address записывается
            // в sector register.
            sector_reg = id_track;

            transfer_pos = 0;
            transfer_len = 6;
            transfer_type = TRANSFER_READ_ADDRESS;
            drq = true;
        ...
        // -------------------------
        // Type III: WRITE TRACK (используется TR-DOS FORMAT)
        // -------------------------
        if ((val & 0xF0) == 0xF0) {
            last_cmd_type1 = false;
            multi_sector = false;
            ...
            busy = false;       // оставляем твою рабочую модель
            intrq = true;       // сохраняем старое состояние bit 7

            transfer_type = TRANSFER_WRITE_TRACK;
            transfer_pos = 0;
            transfer_len = WRITE_TRACK_BYTES;
            drq = true; // готовы принимать первый байт потока трека

            return;
        }

        // -------------------------
        // Type III: READ TRACK — не реализовано (нет побитового образа
        // трека). Явно завершаем командой без данных, чтобы не висеть.
        // -------------------------
        if ((val & 0xF0) == 0xE0) {
            last_cmd_type1 = false;
            EndCommand(!DiskReady());
            return;
        }

        // -------------------------
        // Всё остальное (0xE0-0xFF) — реальные загрузчики TR-DOS/игр
        // сюда не заходят с намерением реально форматировать трек;
        // подтверждено эмпирически: попытка честно реализовать здесь
        // WRITE TRACK (ожидание 6250 байт через порт 0x7F) вешает
        // загрузку, потому что ReadData() эти байты не производит.
        // Оставляем безопасный no-op, не трогая busy/drq/intrq.
        // -------------------------
        last_cmd_type1 = false;
        transfer_pos = -1;
        transfer_len = 256;
    ...
    void WriteTrack(BYTE val, WORD pc) {
        if (!busy) {
            track_reg = val;
        }
    }

    void WriteSector(BYTE val, WORD pc) {
        if (!busy) {
            sector_reg = val;
        }
    }
    ...
    bool FlushCurrentSector() {
        if (!DiskReady() || sector_reg < 1 || sector_reg > 16) return false;
        size_t offset = GetTRDOffset();
        if (offset + 256 > current_trd_data.size()) return false;
        memcpy(&current_trd_data[offset], sector_buffer, 256);
        return true;
    }

    static const int WRITE_TRACK_BYTES = 6250; // приблизительная длина сырого потока трека DD

    // "Логическое" форматирование текущей дорожки/стороны: 16 секторов
    // по 256 нулевых байт. Реального разбора gap/sync/CRC байт WRITE TRACK
    // здесь нет — модель работает с готовым .trd, а не с побитовым треком.
    bool FormatCurrentTrack() {
        ...
        for (int s = 1; s <= 16; ++s) {
            ...
            size_t offset = GetTRDOffset();
            if (offset + 256 > current_trd_data.size()) return false;
            std::fill(current_trd_data.begin() + offset,
                current_trd_data.begin() + offset + 256, 0);
        }
        return true;
    }

    void WriteData(BYTE val, WORD pc) {
        data_reg = val;

        if (transfer_type == TRANSFER_WRITE_TRACK) {
            if (transfer_pos >= 0 && transfer_pos < transfer_len) {
                ++transfer_pos;
                if (transfer_pos >= transfer_len) {
                    bool ok = FormatCurrentTrack();
                    EndCommand(!ok);
                }
                // drq остаётся true всё время передачи — контроллер
                // готов принять очередной байт потока трека сразу.
            }
            return;
        }

        if (transfer_type == TRANSFER_WRITE_SECTOR) {
            if (transfer_pos >= 0 && transfer_pos < transfer_len) {
                sector_buffer[transfer_pos++] = data_reg;
                drq = (transfer_pos < transfer_len);
    ...
    void EndCommand(bool rnf) {
        transfer_pos = -1;
        transfer_len = 0;
        transfer_type = TRANSFER_NONE;
        multi_sector = false;
        record_not_found = rnf;

        busy = false; // Контроллер освободился!
        drq = false;
        intrq = true;
    }

    void WriteSystem(BYTE val, WORD pc) {
        system_reg = val;

        // CLR = 0: сбрасывает только саму микросхему ВГ93 (регистры
        // track/sector/data/статус передачи), сторона/привод из этой
        // же записи (system_reg = val, уже выполнено строкой выше)
        // должны остаться в силе.
        if ((val & 0x04) == 0) {
            ResetChip();
        }
    }
...
struct CPUZ80 {
    ...
    void Reset() {
        ...
        // Сброс диспетчера памяти 128K
        current_ram_page = 0;
        current_rom_page = 0;
        screen_shadow_page = false;
        memory_lock_128k = false;
        trdos_active = false;
    }
    ...
    // --- ПРАВИЛЬНАЯ ШИНА ЧТЕНИЯ ДАННЫХ (БЕЗ СДВИГА ОЗУ И ПРЕЖДЕВРЕМЕННОГО СБРОСА) ---
    inline BYTE ReadByte(int addr) {
        int target = addr & 0xFFFF;

        // --- ТОЧНЫЙ АППАРАТНЫЙ ПЕРЕХВАТ ЦИКЛА M1 ---
        // Перехват срабатывает СТРОГО когда текущий выполняемый адрес (PC) 
        // попадает на вектор TR-DOS (0x3D00 - 0x3FFF), находясь при этом в 48К Бэйсике (ROM 1).
        if (PC >= 0x3D00 && PC <= 0x3FFF && current_rom_page == 1 && !trdos_active) {
            trdos_active = true;
            beta_ports_enabled = true; // включаем доступ к портам и оставляем его включённым
        }
        if (PC >= 0x4000 && trdos_active) {
            trdos_active = false; // страница ПЗУ гасится как обычно
            // beta_ports_enabled НЕ трогаем - порты остаются живыми для загрузчиков в ОЗУ
        }

        // Окно 0: 0x0000 - 0x3FFF (Системное ПЗУ 16 Кб)
        if (target < 0x4000) {
            if (trdos_active) return trdos_rom[target]; // Читаем из ПЗУ TR-DOS
            return spec_rom[current_rom_page][target];   // Читаем из обычного ПЗУ Спектрума
        }

        // Окно 1: 0x4000 - 0x7FFF (Системное ОЗУ, строго Страница RAM 5 — экран)
        if (target < 0x8000) {
            return spec_ram[5][target - 0x4000]; // Явный двумерный индекс банка 5
        }

        // Окно 2: 0x8000 - 0xBFFF (Системное ОЗУ, строго Страница RAM 2)
        if (target < 0xC000) {
            return spec_ram[2][target - 0x8000]; // Явный двумерный индекс банка 2
        }

        // Окно 3: 0xC000 - 0xFFFF (Переключаемая страница ОЗУ 0-7)
        return spec_ram[current_ram_page][target - 0xC000];
    }

    inline void WriteByte(int addr, BYTE val) {
        int target = addr & 0xFFFF;

        // ВАЖНО: При записи данных (WriteByte) флаг trdos_active НЕ сбрасываем!
        // Это позволяет TR-DOS и вашему низкоуровневому хуку в StepZ80 свободно
        // писать сектора и переменные дисковода в ОЗУ Спектрума.

        if (target < 0x4000) {
            return; // Защита от записи в ПЗУ
        }
        if (target < 0x8000) {
            spec_ram[5][target - 0x4000] = val; // Явный двумерный индекс банка 5
            return;
        }
        if (target < 0xC000) {
            spec_ram[2][target - 0x8000] = val; // Явный двумерный индекс банка 2
            return;
        }
        spec_ram[current_ram_page][target - 0xC000] = val; // Пишем в переключаемый банк
    }

    // Ввод из портов с полноценным сопряжением логики ПЗУ TR-DOS и ВГ93
    inline BYTE InPort(WORD port) {
        if (beta_ports_enabled) {   // было: if (trdos_active)
            BYTE low_port = port & 0xFF;
            if (low_port == 0x1F) return vg93.ReadStatus(PC);
            if (low_port == 0x3F) return vg93.ReadTrack(PC);
            if (low_port == 0x5F) return vg93.ReadSector(PC);
            if (low_port == 0x7F) return vg93.ReadData(PC);
            if (low_port == 0xFF) return vg93.ReadSystem(PC);
        }

        // 2. Чтение клавиатуры (стандартный ULA порт 0xFE)
        if ((port & 0xFF) == 0xFE) {
            BYTE result = 0x1F;
            for (int i = 0; i < 8; i++) {
                if (!(port & (1 << (8 + i)))) result &= spec_key_rows[i];
            }
            return result | 0xC0 | (spec_ear_signal ? 0x20 : 0x00);
        }

        // 3. Чтение музыкального регистра AY-3-8910 (0xFFFD)
        // Чтение данных AY-3-8910 (срабатывает на 0xFFFD и его зеркала)
        // Условие: A15=1, A14=1, A1=0. В шестнадцатеричном виде это маска 0xC002, дающая результат 0xC000
    ...
    inline void OutPort(WORD port, BYTE val) {
        ...
        if (beta_ports_enabled) {   // было: if (trdos_active)
            ...
            if (low_port == 0xFF) { vg93.WriteSystem(val, PC); return; }
        }

        // 1. Переключение страниц памяти 128K (Порт 0x7FFD)
        if ((port & 0x8002) == 0x0000) {
            if (!memory_lock_128k) {
                current_ram_page = val & 0x07;
                screen_shadow_page = (val & 0x08) != 0;
                current_rom_page = (val & 0x10) ? 1 : 0;
                if (val & 0x20) memory_lock_128k = true;
            }
            return;
        }

        // 2. Музыкальный чип AY-3-8910 (Порты 0xFFFD и 0xBFFD)
        // Запись в AY-3-8910 (срабатывает на биты A15=1 и A1=0, то есть маска 0x8002 == 0x8000)
        if ((port & 0x8002) == 0x8000) {
            EnterCriticalSection(&audio_cs);

            // Бит A14 определяет, ЧТО мы делаем:
            if (port & 0x4000) {
                // Если A14 == 1 (например, порт 0xFFFD) -> это ВЫБОР регистра
                ay.selected_reg = val & 0x0F;
            }
            else {
                // Если A14 == 0 (например, порт 0xBFFD) -> это ЗАПИСЬ данных в регистр
        ...
        // 3. Стандартный системный порт ULA (0xFE)
        if ((port & 0xFF) == 0xFE) {
...
void RenderSpectrumScreen(HDC hdc, int xOffset, int yOffset) {
    ...
    int video_page = screen_shadow_page ? 7 : 5; // Выбираем страницу ОЗУ для вывода кадра
    ...
    for (int y = 0; y < 192; y++) {
        ...
        for (int x_byte = 0; x_byte < 32; x_byte++) {
            BYTE pixel_data = spec_ram[video_page][pixel_line_addr - 0x4000 + x_byte];

            // Каждому байту пикселей соответствует 1 байт атрибутов цвета
            int attr_addr = 0x5800 + ((y >> 3) * 32) + x_byte;
            BYTE attr = spec_ram[video_page][attr_addr - 0x4000];
...
static bool LoadSnaSnapshot(const std::vector<BYTE>& data, std::wstring& error) {
    // Стандартный размер 48K SNA = 49179. Для 128K SNA = 131103 байта.
    if (data.size() != 49179 && data.size() != 131103) {
        error = L"Неподдерживаемый размер файла .SNA. Должен быть 49179 (48K) или 131103 (128K).";
        return false;
    }
    ...
    // 2. Логика загрузки в зависимости от типа SNA (48K или 128K)
    if (data.size() == 49179) {
        // --- Режим 48K SNA ---
        // Переводим менеджер памяти в режим совместимости 48K
        current_rom_page = 1;      // ПЗУ 48K BASIC
        current_ram_page = 0;      // В окне 0xC000 страница 0
        screen_shadow_page = false;// Экран в RAM 5
        memory_lock_128k = true;   // Блокируем порт 7FFD

        // Раскладываем 48 Кб памяти по фиксированным страницам 128K архитектуры
        // Смещение 0x4000 в 48К — это страница RAM 5
        for (size_t i = 0; i < 16384; ++i) spec_ram[5][i] = data[27 + i];
        // Смещение 0x8000 в 48К — это страница RAM 2
        for (size_t i = 0; i < 16384; ++i) spec_ram[2][i] = data[27 + 16384 + i];
        // Смещение 0xC000 в 48К — это страница RAM 0
        for (size_t i = 0; i < 16384; ++i) spec_ram[0][i] = data[27 + 32768 + i];

        // В 48К SNA адрес PC лежит на вершине стека ( RETN симуляция )
        if (cpu.SP < 0x4000) {
            error = L"Некорректный .SNA 48K: SP указывает в область ПЗУ.";
            return false;
        }
        cpu.PC = (WORD)cpu.ReadByte(cpu.SP) | ((WORD)cpu.ReadByte(cpu.SP + 1) << 8);
        cpu.SP = (WORD)(cpu.SP + 2);
    }
    else {
        // --- Режим 128K SNA ---
        // Извлекаем расширенный заголовок 128К из хвоста файла (смещение 27 + 49152 = 49179)
        size_t ext_offset = 49179;
        cpu.PC = ReadLE16(data.data() + ext_offset); // PC записан в расширенном заголовке напрямую
        BYTE port_7ffd_val = data[ext_offset + 2];
        bool trdos_flag = data[ext_offset + 3] != 0; // Состояние интерфейса TR-DOS

        // Применяем конфигурацию портов 128K системы
        current_ram_page = port_7ffd_val & 0x07;
        screen_shadow_page = (port_7ffd_val & 0x08) != 0;
        current_rom_page = (port_7ffd_val & 0x10) ? 1 : 0;
        memory_lock_128k = (port_7ffd_val & 0x20) != 0;
        trdos_active = trdos_flag;

        // Сначала восстанавливаем стандартные окна из основного дампа файла
        for (size_t i = 0; i < 16384; ++i) spec_ram[5][i] = data[27 + i];          // Окно 0x4000 (RAM 5)
        for (size_t i = 0; i < 16384; ++i) spec_ram[2][i] = data[27 + 16384 + i];  // Окно 0x8000 (RAM 2)
        for (size_t i = 0; i < 16384; ++i) spec_ram[current_ram_page][i] = data[27 + 32768 + i]; // Окно 0xC000

        // Затем считываем оставшиеся страницы ОЗУ из хвоста файла (каждая по 16 Кб).
        // Порядок страниц в файле фиксирован: RAM 0, 1, 2, 3, 4, 5, 6, 7.
        // Но те страницы, которые мы уже прочитали (5, 2 и текущая), в хвосте файла заполнены нулями/дублированы, 
        // поэтому мы безопасно перезапишем или пропустим их.
        size_t page_data_ptr = ext_offset + 4;
        for (int p = 0; p < 8; p++) {
            // Пропускаем чтение страниц 5 и 2 из хвоста, чтобы не затереть активные окна
            if (p != 5 && p != 2 && p != current_ram_page) {
                if (page_data_ptr + 16384 <= data.size()) {
                    for (size_t i = 0; i < 16384; ++i) {
                        spec_ram[p][i] = data[page_data_ptr + i];
                    }
                }
            }
            page_data_ptr += 16384; // Смещаемся по архиву на следующую страницу в любом случае
        }
    }
...
static bool LoadZ80Snapshot(const std::vector<BYTE>& data, std::wstring& error) {
    ...
    if (pc_header != 0) {
        // --- ОБРАБОТКА ФОРМАТА .Z80 v1 (Строго 48K режим) ---
        current_rom_page = 1;
        current_ram_page = 0;
        screen_shadow_page = false;
        memory_lock_128k = true;
        trdos_active = false;

        const bool compressed = (h[12] & 0x20) != 0;
        std::vector<BYTE> ram;
        if (compressed) {
            if (!DecompressZ80(data.data() + 30, data.size() - 30, ram, 49152, true)) {
        ...
        // Переносим данные из линейного дампа 48К в страничную структуру 128К
        for (size_t i = 0; i < 16384; ++i) spec_ram[5][i] = ram[i];
        for (size_t i = 0; i < 16384; ++i) spec_ram[2][i] = ram[16384 + i];
        for (size_t i = 0; i < 16384; ++i) spec_ram[0][i] = ram[32768 + i];
        cpu.PC = pc_header;
    }
    else {
        // --- ОБРАБОТКА ФОРМАТА .Z80 v2/v3 (Поддержка 128K и TR-DOS) ---
        if (data.size() < 32) {
        ...
        cpu.PC = ReadLE16(ext); // Настоящий PC для версий v2/v3

        BYTE hardware_mode = ext[2];
        // В формате .Z80 режимы 3 и 4 соответствуют классическому ZX Spectrum 128K
        // Восстанавливаем состояние аппаратного регистра переключения памяти из байта порта 7FFD (ext[ trace_offset ])
        BYTE port_7ffd_val = ext[5];
        ...
        // Если snapshot был сделан внутри интерфейса TR-DOS (определяется по аппаратной конфигурации v3)
        if (ext_len >= 54) {
            BYTE port_1ffd_val = ext[49]; // Дополнительный порт для модификаций +3/TR-DOS
            // Если в файле зафиксирована активность TR-DOS ПЗУ
            if (hardware_mode == 12 || hardware_mode == 13 || (ext[48] & 0x01)) {
                trdos_active = true;
            }
        }

        // Очищаем все страницы расширенного ОЗУ перед заполнением
        for (int p = 0; p < 8; p++) std::fill(spec_ram[p], spec_ram[p] + 16384, 0);

        size_t pos = 32u + ext_len;
        while (pos + 3 <= data.size()) {
            WORD block_len = ReadLE16(data.data() + pos);
            BYTE page_id = data[pos + 2]; // Идентификатор страницы в .Z80 формате
            pos += 3;

            size_t stored_size = (block_len == 0xFFFF) ? 16384u : (size_t)block_len;
            if (pos + stored_size > data.size()) {
                error = L"Повреждённый блок RAM в структуре файла .Z80.";
                return false;
            }

            // Маппинг страниц формата .Z80 в страницы памяти нашего эмулятора 128К
            int target_page_index = -1;
            if (page_id >= 3 && page_id <= 10) {
                target_page_index = page_id - 3; // Страницы 3..10 соответствуют RAM 0..7
            }
            else if (page_id == 0) {
                // Если snapshot v2 хранит старый 48K маркер ОЗУ 0x4000
                target_page_index = 5;
            }

            if (target_page_index >= 0 && target_page_index < 8) {
                std::vector<BYTE> block;
                if (block_len == 0xFFFF) {
                    block.assign(data.begin() + pos, data.begin() + pos + 16384);
                }
                else {
                    if (!DecompressZ80(data.data() + pos, block_len, block, 16384, false)) {
                        error = L"Не удалось распаковать блок RAM в .Z80.";
                        return false;
                    }
                }
                // Копируем распакованную страницу в соответствующий банк ОЗУ 128К
                std::copy(block.begin(), block.end(), spec_ram[target_page_index]);
            }
...
static bool LoadTapFile(const std::vector<BYTE>& data, std::wstring& error) {
    ...
    // 1. Сохраняем образ ленты в глобальную память эмулятора
    current_tap_data = data;
    tap_current_pos = 0;
    tap_is_loaded = true;

    // 2. Полный аппаратный сброс процессора Z80
    cpu.Reset();

    // 3. Полная очистка всех 8 страниц оперативной памяти 128K
    for (int p = 0; p < 8; p++) {
        std::fill(spec_ram[p], spec_ram[p] + 16384, 0);
    }

    // 4. Сброс состояния звукового кольцевого буфера и музыкального чипа AY
    ay.Reset();

    // 5. Перевод менеджера памяти в начальное состояние ZX Spectrum 128K
    current_ram_page = 0;     // В окне 0xC000-0xFFFF находится страница RAM 0
    current_rom_page = 0;     // В окне 0x0000-0x3FFF находится ПЗУ 128K Меню
    screen_shadow_page = false; // Активный экран расположен в RAM 5
    memory_lock_128k = false;   // Порт 0x7FFD открыт для переключения страниц
    trdos_active = false;       // Отключаем ПЗУ TR-DOS до системных вызовов

    // 6. Инициализируем матрицу клавиатуры по умолчанию (1 = клавиши отжаты)
    for (int i = 0; i < 8; i++) {
    ...
    // Система готова. При старте эмулятор покажет фирменное меню ZX Spectrum 128.
    // Пользователю достаточно выбрать пункт "Tape Loader" (или войти в "128 BASIC" и ввести LOAD ""), 
    // после чего ROM-хук на адресе 0x0556 мгновенно загрузит игру из вектора current_tap_data.

    return true;
}

static bool LoadTrdFile(const std::vector<BYTE>& data, std::wstring& error) {
    if (data.empty()) {
        error = L"Файл TRD пуст.";
        return false;
    }

    current_trd_data = data;
    if (current_trd_data.size() < 655360) {
        current_trd_data.resize(655360, 0);
        size_t sys_offset = 2048; // track 0, sector 8 (0-based) — РЕАЛЬНОЕ расположение дескриптора TR-DOS
        current_trd_data[sys_offset + 225] = 0x16; // 80 дорожек, 2 стороны
        current_trd_data[sys_offset + 226] = 0x01;
    }

    trd_is_inserted = true;
    vg93.Reset();
    ...
    for (int i = 0; i < 8; i++) {
        spec_key_rows[i] = 0x1F; physical_key_rows[i] = 0x1F; gui_key_rows[i] = 0x1F;
    }
    ...
    const std::wstring& original_path)
{
    ...
    if (data.size() < 13)
    {
        error = L"Файл SCL поврежден или имеет неверный размер.";
        return false;
    }
    ...
    {
        error =
            L"Неверный формат SCL "
            L"(отсутствует сигнатура SINCLAIR).";

        return false;
    }

    const BYTE num_files = data[8];

    constexpr size_t SCL_HEADER_SIZE = 9;
    constexpr size_t SCL_FILE_HEADER = 14;
    constexpr size_t SCL_CHECKSUM_SIZE = 4;
    ...
    if (data.size() < expected_headers_size +
        SCL_CHECKSUM_SIZE)
    {
        error = L"Каталог файлов SCL поврежден.";
        return false;
    }

    // Последние 4 байта SCL — checksum.
    const size_t scl_data_end =
        data.size() - SCL_CHECKSUM_SIZE;

    // ============================================================
    // 2. Создаем TRD
    // ============================================================

    constexpr size_t TRD_SIZE = 655360;
    constexpr size_t SECTOR_SIZE = 256;
    constexpr size_t SECTORS_PER_TRACK = 16;

    constexpr size_t TRD_TOTAL_SECTORS =
        TRD_SIZE / SECTOR_SIZE;       // 2560

    constexpr size_t TRD_DATA_SECTORS =
        TRD_TOTAL_SECTORS - 16;       // 2544

    std::vector<BYTE> trd_image(
        TRD_SIZE,
        0);
    ...
    constexpr size_t sys_sector_offset =
        8 * SECTOR_SIZE;

    // ВАЖНО:
    // +225 = первый свободный сектор
    // +226 = первая свободная дорожка
    // +227 = тип диска
    // +228 = количество файлов
    // +229/+230 = свободные сектора
    // +231 = 16 секторов на дорожке

    trd_image[sys_sector_offset + 227] =
        0x16; // 80 tracks / double sided

    trd_image[sys_sector_offset + 231] =
        0x10; // 16 sectors per track

    // Метка диска — 8 байт.
    memcpy(
        &trd_image[sys_sector_offset + 245],
        "SCL_DISK",
        8);

    // ============================================================
    // 4. Начальная позиция файлов
    //
    // КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ:
    //
    // Первый файл находится:
    //
    //     Track 1 / Sector 0
    //
    // а не Track 1 / Sector 1.
    // ============================================================

    BYTE current_sector = 0;
    BYTE current_track = 1;

    BYTE total_files_copied = 0;

    size_t scl_file_data_ptr =
    ...
    for (BYTE f = 0;
        ...
        ++f)
    {
        const size_t scl_header_ptr =
            SCL_HEADER_SIZE +
            static_cast<size_t>(f) *
            SCL_FILE_HEADER;

        // --------------------------------------------------------
        // Данные SCL-заголовка
        // --------------------------------------------------------

        const BYTE file_type =
            data[scl_header_ptr + 8];

        const WORD file_param1 =
            static_cast<WORD>(
                data[scl_header_ptr + 9] |
                (
                    static_cast<WORD>(
                        data[scl_header_ptr + 10])
                    << 8
                    ));

        const WORD file_param2 =
            static_cast<WORD>(
                data[scl_header_ptr + 11] |
                (
                    static_cast<WORD>(
                        data[scl_header_ptr + 12])
                    << 8
                    ));

        const BYTE file_sectors =
            data[scl_header_ptr + 13];

        // --------------------------------------------------------
        // Размер, занимаемый файлом в SCL/TRD
        // --------------------------------------------------------

        const size_t file_storage_size =
            static_cast<size_t>(file_sectors) *
            SECTOR_SIZE;

        if (scl_file_data_ptr > scl_data_end ||
            file_storage_size >
            scl_data_end - scl_file_data_ptr)
        {
            error =
                L"Данные файла выходят "
                L"за пределы SCL.";
        ...
        const size_t used_sectors =
            (static_cast<size_t>(current_track) - 1) *
            SECTORS_PER_TRACK +
            static_cast<size_t>(current_sector);

        if (used_sectors > TRD_DATA_SECTORS ||
            static_cast<size_t>(file_sectors) >
            TRD_DATA_SECTORS - used_sectors)
        {
            error =
                L"Файлы SCL не помещаются "
                L"на TRD.";
        ...
        // --------------------------------------------------------
        // Запись каталога TR-DOS
        //
        // Один descriptor = 16 байт.
        // --------------------------------------------------------

        if (total_files_copied >= 128)
        {
            error =
                L"Каталог TR-DOS переполнен.";

            return false;
        }

        const size_t trd_catalog_ptr =
            static_cast<size_t>(
                total_files_copied) * 16;

        // Имя — 8 байт.
        memcpy(
            &trd_image[trd_catalog_ptr],
            &data[scl_header_ptr],
            8);

        // Тип.
        trd_image[trd_catalog_ptr + 8] =
            file_type;

        // Параметр 1.
        trd_image[trd_catalog_ptr + 9] =
            static_cast<BYTE>(
                file_param1 & 0xFF);

        trd_image[trd_catalog_ptr + 10] =
            static_cast<BYTE>(
                (file_param1 >> 8) & 0xFF);

        // Параметр 2.
        trd_image[trd_catalog_ptr + 11] =
            static_cast<BYTE>(
                file_param2 & 0xFF);

        trd_image[trd_catalog_ptr + 12] =
            static_cast<BYTE>(
                (file_param2 >> 8) & 0xFF);

        // Количество секторов.
        trd_image[trd_catalog_ptr + 13] =
            file_sectors;

        // ========================================================
        // КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ:
        //
        // Sector в TR-DOS каталоге = 0..15.
        // ========================================================

        trd_image[trd_catalog_ptr + 14] =
            current_sector;

        // Track.
        trd_image[trd_catalog_ptr + 15] =
            current_track;
        ...
        //
        // ========================================================
        // НЕ используем current_sector - 1.
        // --------------------------------------------------------

        const size_t trd_data_offset =
            (
                static_cast<size_t>(current_track) *
                SECTORS_PER_TRACK +
                static_cast<size_t>(current_sector)
                ) * SECTOR_SIZE;

        if (trd_data_offset > trd_image.size() ||
            file_storage_size >
            trd_image.size() - trd_data_offset)
        {
            error =
                L"Файл выходит за пределы TRD-образа.";
        ...
        //
        // ВАЖНО:
        // копируем весь выделенный SCL объем,
        // а не только file_len.
        // --------------------------------------------------------

        if (file_storage_size != 0)
        {
            memcpy(
                &trd_image[trd_data_offset],
                &data[scl_file_data_ptr],
                file_storage_size);
        }
        ...
        while (next_sector >= SECTORS_PER_TRACK)
        {
            next_sector -= SECTORS_PER_TRACK;
            ++current_track;
        }

        current_sector =
            static_cast<BYTE>(next_sector);

        // --------------------------------------------------------
        // Следующий блок данных SCL.
        // --------------------------------------------------------
    ...
    // ============================================================
    // 6. Системная информация TR-DOS
    // ============================================================

    // Первый свободный сектор.
    trd_image[sys_sector_offset + 225] =
        current_sector;

    // Первая свободная дорожка.
    trd_image[sys_sector_offset + 226] =
        current_track;
    ...
    // Количество файлов.
    trd_image[sys_sector_offset + 228] =
        total_files_copied;

    // ------------------------------------------------------------
    // Количество занятых data-секторов
    // ------------------------------------------------------------

    const size_t used_data_sectors =
        (static_cast<size_t>(current_track) - 1) *
        SECTORS_PER_TRACK +
        static_cast<size_t>(current_sector);

    if (used_data_sectors > TRD_DATA_SECTORS)
    {
        error =
            L"Ошибка расчета свободного места TRD.";

        return false;
    }

    const WORD free_sectors =
        static_cast<WORD>(
            TRD_DATA_SECTORS -
            used_data_sectors);

    trd_image[sys_sector_offset + 229] =
        static_cast<BYTE>(
            free_sectors & 0xFF);

    trd_image[sys_sector_offset + 230] =
        static_cast<BYTE>(
            (free_sectors >> 8) & 0xFF);
    ...
    {
        std::wstring trd_path =
            original_path;

        const size_t dot =
            trd_path.find_last_of(L'.');

        if (dot != std::wstring::npos)
        {
            trd_path =
                trd_path.substr(0, dot) +
                L".trd";
        }
        else
        {
    ...
    // ============================================================
    // 8. Передаем готовый TRD обработчику
    // ============================================================
...
// Загрузка снимков памяти, лент или образов дисков ZX Spectrum
void LoadSpectrumFile(HWND hwnd) {
    ...
    // Расширен фильтр файлов для отображения всех поддерживаемых 128K/TR-DOS форматов
    ofn.lpstrFilter =
        L"Файлы ZX Spectrum (*.sna;*.z80;*.tap;*.scl;*.trd)\0*.sna;*.z80;*.tap;*.scl;*.trd\0"
    ...
    if (!file.is_open()) {
        MessageBoxW(hwnd, L"Не удалось открыть выбранный файл.",
            L"Ошибка ввода-вывода", MB_OK | MB_ICONERROR);
        return;
    }
    ...
    // Переводим расширение в нижний регистр для безопасного сравнения
    for (wchar_t& c : ext) {
    ...
    // Распределение файлов по специализированным загрузчикам
    if (ext == L".sna") {
        ...
        ok = LoadSclFile(data, error, path);
    }
    else if (ext == L".trd") { // <-- ДОБАВЛЕННАЯ ВЕТКА ДЛЯ TRD ДИСКОВ
        ok = LoadTrdFile(data, error);
    }
    else {
        // Резервный загрузчик для сырых бинарных дампов (.BIN / .ROM)
        if (data.empty()) {
            error = L"Файл бинарного дампа пуст.";
        }
        else {
            // Полный сброс процессора Z80
            cpu.Reset();

            // Очищаем все 8 страниц ОЗУ 128К перед копированием дампа
            for (int p = 0; p < 8; p++) {
                std::fill(spec_ram[p], spec_ram[p] + 16384, 0);
            }

            // Сбрасываем диспетчер страниц памяти в стандартный режим 128К
            current_ram_page = 0;
            current_rom_page = 0;
            screen_shadow_page = false;
            memory_lock_128k = false;
            ...
            // Копируем сырые данные. Максимальный размер ограничен окном ОЗУ (16 Кб)
            size_t copy_size = (data.size() > 16384) ? 16384 : data.size();

            // Копируем в страницу 0, которая по умолчанию отображается на адрес 0xC000
            std::copy(data.begin(), data.begin() + copy_size, spec_ram[0]);

            // Настраиваем точку старта и указатель стека
            cpu.PC = 0xC000;
            cpu.SP = 0xFFFF;
            ok = true;
        }
    ...
    wchar_t title[320];
    swprintf_s(title, 320, L"Эмулятор ZX Spectrum 128 (TR-DOS Активен) — %s", szFile);
...
static void PollPhysicalKeyboard(HWND hwnd) {
    ...
    if (GetAsyncKeyState(VK_LEFT) & 0x8000) {  // Стрелка Влево = Caps + 5
        SetPhysicalMatrixBit(3, 4, true);
        force_caps_shift = true;
    }
    else if (!(GetAsyncKeyState('5') & 0x8000)) {
        SetPhysicalMatrixBit(3, 4, false);
    }

    if (GetAsyncKeyState(VK_DOWN) & 0x8000) {  // Стрелка Вниз = Caps + 6
        SetPhysicalMatrixBit(4, 4, true);
        force_caps_shift = true;
    }
    else if (!(GetAsyncKeyState('6') & 0x8000)) {
        SetPhysicalMatrixBit(4, 4, false);
    }

    if (GetAsyncKeyState(VK_UP) & 0x8000) {    // Стрелка Вверх = Caps + 7
        SetPhysicalMatrixBit(4, 3, true);
        force_caps_shift = true;
    }
    else if (!(GetAsyncKeyState('7') & 0x8000)) {
        int r_7 = spec_hardware_map[6].row; int b_7 = spec_hardware_map[6].bit;
        SetPhysicalMatrixBit(r_7, b_7, (GetAsyncKeyState('7') & 0x8000) != 0);
    }

    if (GetAsyncKeyState(VK_RIGHT) & 0x8000) { // Стрелка Вправо = Caps + 8
        SetPhysicalMatrixBit(4, 2, true);
        force_caps_shift = true;
    }
    else if (!(GetAsyncKeyState('8') & 0x8000)) {
        SetPhysicalMatrixBit(4, 2, false);
    }
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_COMMAND: {
        ...
        if (btn_id == 2000) { // Сброс
            ...
            for (int i = 0; i < 8; i++) std::fill(spec_ram[i], spec_ram[i] + 16384, 0);
            for (int i = 0; i < 8; i++) { spec_key_rows[i] = 0x1F; physical_key_rows[i] = 0x1F; gui_key_rows[i] = 0x1F; }
            for (int i = 0; i < 40; ++i) gui_key_frames[i] = 0;
            beta_ports_enabled = false;
            spec_flash_frame = 0;
            spec_flash_state = false;
            UpdateRegisterDisplay();
            SetFocus(hwnd);
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int nCmdShow) {
    ...
    memset(spec_rom, 0x00, sizeof(spec_rom));

    // Загрузка ROM 0 (128K Menu/Editor)
    std::ifstream rom0("ZXSpectrum128_0.rom", std::ios::binary);
    if (rom0.is_open()) { rom0.read((char*)spec_rom[0], 16384); rom0.close(); }

    // Загрузка ROM 1 (48K BASIC)
    std::ifstream rom1("ZXSpectrum128_1.rom", std::ios::binary);
    if (rom1.is_open()) { rom1.read((char*)spec_rom[1], 16384); rom1.close(); }
    else { // Если файлы 128К не найдены, сохраняем обратную совместимость с вашим старым 48К дампом
        std::ifstream rom_old("ZXSpectrum48.rom", std::ios::binary);
        if (rom_old.is_open()) { rom_old.read((char*)spec_rom[1], 16384); rom_old.close(); }
    }

    // Загрузка ПЗУ TR-DOS
    std::ifstream trdos_file("TRDOS.rom", std::ios::binary);
    if (trdos_file.is_open()) { trdos_file.read((char*)trdos_rom, 16384); trdos_file.close(); }
